...
class NaturalCOLD:
    ...
    def optimize(self, documents, trigger, start_tokens=None, lr=1, topk=10, beam_width=10, eps=0.1, stemp=0.5, epoch_num=3, perturb_iter=10): 
        print("stemp", stemp)
        @dataclass
        class BeamCandidate:
        ...
        def beam_search(z_i, num_tokens, doc_embs, trig_doc_embs, topk, beam_width, stemp):
            ...
            for i in tqdm(range(num_tokens)):
                ...
                for j, candidate in enumerate(batch_sequences):
                    ...
                    all_candidates.append(BeamCandidate(new_sequence, score_fn(current_trig_sim, current_naturalness, current_perplexity), current_trig_sim, current_naturalness, current_perplexity))
        ...
        for epoch in range(epoch_num):
            ...
            for i in range(perturb_iter):
                ...
                loss = 1 - score_fn(trig_cos_sim, naturalness, perplexity)
                print(epoch, i, 'naturalness', naturalness.item(), 'trig_sim', trig_cos_sim.item(), 'perplexity', perplexity.item(), 'loss', loss.item(), flush=True)
                loss.backward()
                optimizer.step()
            ...
            z_i = z_i.detach()
            print(z_i)
            candidate: BeamCandidate = beam_search(z_i, len(z_i), None, trig_doc_embs, topk, beam_width, stemp)
